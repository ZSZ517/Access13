...
public class s2RecognizeTest extends HaveToolBarActivity {
	...
	protected void onCreate(Bundle bundle) {
		...
		setContentView(R.layout.ac2tivity_recognize_test);
		getSupportActionBar().setTitle("识别测试");
		initView();
		screenWidth = CommonSize.getScreenWidth(s2RecognizeTest.this);
	}

	protected void onRestart() {
		super.onRestart();
		// finish();
		// Intent intent = new Intent(s2RecognizeTest.this, s2RecognizeTest.class);
	...
	private CameraDevice.StateCallback stateCallback = new CameraDevice.StateCallback() {
		...
		public void onDisconnected(CameraDevice camera) {// 关闭摄像头
			...
			if (null != mCameraDevice) {// 如果不判空，当cameraDevice为空时就会抛出异常
				...
				s2RecognizeTest.this.mCameraDevice = null;
		...
		public void onError(CameraDevice camera, int error) {
			...
			Toast.makeText(s2RecognizeTest.this, "摄像头遭遇错误", Toast.LENGTH_SHORT).show();
		}
	};

	// 初始化view并监听组件，因为用内部类和要实现监听的方法，所以该方法较长，但逻辑、界限是分明的。只包括初始化和监听【可能会自动回调其他方法】
	private void initView() {
		button = f(R.id.take_btn);
		...
		mImageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() { // 可以在这里处理拍照得到的临时照片
			...
			public void onImageAvailable(ImageReader reader) {
				...
				if (bitmapOriginal != null) {
					...
					// 剪裁bitmapVertical
					double beginHeightPixel = CommonSize.getDensity(s2RecognizeTest.this) * 20;
				...
				picturePath = new SaveBitmap(s2RecognizeTest.this).saveImageToGallery(bitmapCut);
				showWaitingProgress = new ShowWaitingProgress();
				showWaitingProgress.show(s2RecognizeTest.this, "识别中");
				image.close();
			...
			public void run() {
				...
				try {
					...
					try {
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								// TODO 自动生成的方法存根
								showWaitingProgress.dismiss();
								CommonUtil.showToast(s2RecognizeTest.this, "网络有点卡");
								try {
									...
									// TODO 自动生成的 catch 块
									e.printStackTrace();
									CommonUtil.showToast(s2RecognizeTest.this, "未知原因");
								}
							}
						});
						return;
					...
					try {
						...
						// TODO: handle exception
						CommonUtil.showToast(s2RecognizeTest.this, "response不可以转换为string");
						return;
					}
					String requestSuccess = CommonUtil.requestSuccess;
					if (responseContent.contains(requestSuccess)) {
						// 如果识别成功，则返回识别信息
						Intent intent = new Intent(s2RecognizeTest.this, RecognizeResultActivity.class);
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								CommonUtil.showToast(s2RecognizeTest.this,
						...
						CommonUtil.showToast(s2RecognizeTest.this, JsonCode.getValue(responseContent, "$.message"));
					}
				} catch (Exception e) {
					// TODO: handle exception
					CommonUtil.showToast(s2RecognizeTest.this, "发生未知错误");
				}
			}
		}).start();
	}
	...
	private void startPreview() {
		...
		// CommonUtil.showToast(this, mPreviewsize[0]+" "+mPreviewsize[1]);
			...
			mCameraDevice.createCaptureSession(Arrays.asList(textureSurface, mImageReader.getSurface()),
					new CameraCaptureSession.StateCallback() // 因为后期的拍照要使用imageReader的surface，且拍照的时候还是这个session对象。
					{
						...
						public void onConfigureFailed(CameraCaptureSession cameraCaptureSession) {
							Toast.makeText(s2RecognizeTest.this, "配置失败", Toast.LENGTH_SHORT).show();
						}
					}, childHandler);
		} catch (CameraAccessException e) {
			e.printStackTrace();
